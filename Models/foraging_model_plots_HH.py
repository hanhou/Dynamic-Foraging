...
def plot_one_session(bandit, plottype='2lickport'):     # Part of code from Marton
    ...
    # maximum_reward_rate = np.sum(np.any(bandit.reward_available, axis = 0)) / bandit.n_trials  #??? Method 3: Maximum reward given the fixed reward_available (one choice per trial constraint) [Sugrue 2004???]
    # maximum_reward_rate = np.sum(np.sum(bandit.reward_available, axis = 0)) / bandit.n_trials  #??? Method 4: Sum of all ever-baited rewards (not fair)  
